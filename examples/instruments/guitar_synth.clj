...
;; ======================================================================
;; smoke on the water intro.  turn it up!
(defn ddd0 []
  (let [t (now) dt 250]
    (guitar-strum g [0 2 2 -1 -1 -1] :down 0.01 (+ t (* 0 dt)))
    (guitar-strum g [3 5 5 -1 -1 -1] :up   0.01 (+ t (* 2 dt)))
    (guitar-strum g [5 7 7 -1 -1 -1] :down 0.01 (+ t (* 4 dt)))))
(defn ddd1 []
  (let [t (now) dt 250]
    (guitar-strum g [0 2 2 -1 -1 -1] :down 0.01 (+ t (* 0 dt)))
    (guitar-strum g [3 5 5 -1 -1 -1] :up   0.01 (+ t (* 2 dt)))
    (guitar-strum g [6 8 8 -1 -1 -1] :down 0.01 (+ t (* 4 dt)))
    (guitar-strum g [5 7 7 -1 -1 -1] :down 0.01 (+ t (* 5 dt)))))
(defn ddd2 []
  (let [t (now) dt 250]
    (guitar-strum g [0 2 2 -1 -1 -1] :down 0.01 (+ t (* 0 dt)))
    (guitar-strum g [3 5 5 -1 -1 -1] :up   0.01 (+ t (* 2 dt)))
    (guitar-strum g [5 7 7 -1 -1 -1] :down 0.01 (+ t (* 4 dt)))
    (guitar-strum g [3 5 5 -1 -1 -1] :up   0.01 (+ t (* 7 dt)))
    (guitar-strum g [0 2 2 -1 -1 -1] :down 0.01 (+ t (* 9 dt)))))
;; give us a good, crunchy sound
(ctl g :pre-amp 5.0 :distort 0.99
